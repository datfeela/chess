...
import { Pieces, SquareNum, Square as SquareType, PieceColor } from "../../../redux/chessSlice";
...
export const Board: React.FC = () => {
    ...
    const activatePiece = ({ currentSquare, type, color, name }: ActivatePieceProps) => {
        setActivePiece({ name: name, color: color })
        const possibleMoves = checkForPieceMoves(
            { currentSquare: currentSquare, type: type, color: color, blackPiecesPositions: blackPiecesPositions, whitePiecesPositions: whitePiecesPositions }
        );
    ...
    Object.entries(pieces.white).forEach((piece) => {
        whitePieces.push(
            <Piece key={`white${piece[0]}`} name={piece[0] as keyof Pieces} type={piece[1].type} square={piece[1].square} color={'white'} activatePiece={activatePiece} />
        )
    })
    Object.entries(pieces.black).forEach((piece) => {
        blackPieces.push(
            <Piece key={`white${piece[0]}`} name={piece[0] as keyof Pieces} type={piece[1].type} square={piece[1].square} color={'black'} activatePiece={activatePiece} />
        )
    })
    ...
    for (let y = 8; y > 0; y--) {
        ...
        for (let x = 1; x < 9; x++) {
            ...
            isSquareWhite = !isSquareWhite
            squares.push(
                <Square key={`${x}${y}`} isSquareWhite={isSquareWhite} isSquareActive={isSquareActive} x={x as SquareNum} y={y as SquareNum} />
            )
        }
    }
